...
local function drive_auto_input()
	...
	elseif auto_input_stage == 4 and frame_count > auto_input_at + 900 then
		...
		-- it). Once at the post-invite "PLAYER N OF 4" recruiting screen,
		-- test whether pressing 2P Start is what the game is actually
		-- waiting for (see COMM_PROTOCOL_SPEC.md section 9 -- the comm
		-- pump appeared to go fully idle there, which could just mean
		-- it's correctly waiting on real input we never provided).
		local ok, err = pcall(function()
...
local function do_frame()
	...
	if (frame_count % 60) == 0 then
		...
		if ok then
			...
			-- TCB scan: 58 slots x 64 bytes at $ffd100 (see
			-- COMM_PROTOCOL_SPEC.md's task-scheduler notes). offset+0
			-- (word) nonzero = slot active, offset+2 (long) = routine
			-- pointer. Checking whether $018700 (the low-level comm
			-- pump) is still installed anywhere once things freeze.
			local tok, tlist = pcall(function()
				local space = manager.machine.devices[':maincpu'].spaces['program']
				local entries = {}
				for i = 0, 57 do
					local base = 0xffd100 + i * 0x40
					local active = space:read_u16(base)
					if active ~= 0 then
						local routine = space:read_u32(base + 2)
						entries[#entries + 1] = string.format('[%d]=%04x->%06x', i, active, routine)
					end
				end
				return table.concat(entries, ' ')
			end)
			log('  [tcb] ' .. (tok and tlist or ('scan FAILED: ' .. tostring(tlist))))
		else
