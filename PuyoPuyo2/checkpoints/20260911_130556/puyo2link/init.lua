...
-- or stream positions. RX still bypasses the mailbox copy loop, but follows
-- the ROM's 255-byte maximum occupancy ($18798-$187ae).
...
local function on_write(offset, data, mem_mask)
	...
	if addr == ADDR_CONTROL then
		if byte == 1 then
			...
			log(string.format('[reset] discarded tx=%d rx=%d', #out_queue, #in_queue))
			out_queue = {}
			in_queue = {}
...
local function on_read(offset, data, mem_mask)
	...
	if addr == ADDR_RXCREDIT and established_seen and current_bank == 'cmd' then
		...
		-- Inject at the ROM's RX-copy point in its interrupt, NOT at a
		-- frame boundary that can bisect a consumer's pending-count RMW.
		inject_rx_ring()
	...
	if current_bank == 'tx' and established_seen then
		return tx_bank[bank_block * 8 + idx] or 0
	end
...
local function flush_and_poll()
	if #out_queue > 0 then
		...
		if f then
			...
			local wrote, err = f:write(table.concat(chars))
			local closed, close_err = f:close()
			if not wrote or not closed then
				-- A partial append cannot safely be retried without duplicate
				-- bytes. Stop the bridge and let the real watchdog fail.
				log('[transport-error] append failed: ' .. tostring(err or close_err))
				active = false
				return
			end
			stats.flushed = stats.flushed + #out_queue
...
local function do_prestart()
	...
	io_error_seen, rx_error_seen = false, false
	...
	packet_tap = space:install_write_tap(0xffa514, 0xffa515, 'puyo2link_packets', function(off, data, mask)
		...
		if (mask & 0xff) ~= 0 and pc >= 0x18456 and pc <= 0x1845c then
			...
			if (flags & 0x3c) ~= 0 or stats.accepted <= 4 or (stats.accepted % 60) == 0 then
				log(string.format('[packet-accepted] total=%d seq=%02x flags=%02x status=%02x pc=%06x',
					stats.accepted, ((data & 0xff) - 1) & 0xff, flags,
					space:read_u8(0xffa517), pc))
			end
		end
		return nil
	end)

	gameplay_tap = space:install_write_tap(0xffa530, 0xffa531, 'puyo2link_gameplay', function(off, data, mask)
...
inject_rx_ring = function()
	...
	local pending = space:read_u16(RAM_RX_PEND)
	local head = space:read_u16(RAM_RX_HEAD)
	if pending > 255 or head > 255 then
		if not rx_error_seen then
			log(string.format('[ring-error] refusing injection head=%04x pending=%04x', head, pending))
			rx_error_seen = true
		end
		return
	end
	local room = 255 - pending
...
local function do_frame()
	...
	if (frame_count % 60) == 0 then
		...
		if ok then
			...
			local space = manager.machine.devices[':maincpu'].spaces['program']
			log(string.format('[transport] captured=%d flushed=%d injected=%d accepted=%d payload2=%d payload3=%d payload4=%d payload5=%d pump_reads=%d bursts=%d txcredit=%d outq=%d inq=%d watchdog=%d txseq=%02x rxseq=%02x',
				...
				stats.pump_reads, stats.tx_bursts, #out_queue, #out_queue, #in_queue,
				space:read_u16(0xffa504), space:read_u8(0xffa511), space:read_u8(0xffa515)))
			local game = string.format('a142=%02x a13e=%02x a13f=%02x a530=%04x a506=%02x a507=%02x a508=%02x a7b0=%04x a7a6=%04x script=%06x',
