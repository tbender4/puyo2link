...
**Latest: plugin 0.6.0 + read-only setup diagnostics, spec Update 10
(section 15). MAME runs.**
...
- **303 callback/ROM assertions pass** with the existing GENie host.
...
- **The manual setup difference is now traced and retested, not ignored.**
  Its final `$84,$02` menu selection maps through `$23f66/$1ab8e` to
  AUTO parameter preset 4 (base 140); the earlier exercise's `$84,$00`
  maps to preset 6 (base 120). `$1ab88` enters the settings initializer
  at **`$2444`**, so the base can change through normal gameplay;
  do not blame service/NVRAM tampering or elapsed time alone.
  Both are genuine match 4/mask 0f, not a two-player control.
- `run_manualprofile140_20260911_15`: **100-second input-only test of
  the actual recorded menu selection**, original ROM and divisor 140.
  **10/9 positive A→B/B→A attacks**, all accepted, native accumulator
  writes, landed garbage in all four fields (frame 3000: A 1/1, B 2/1).
  **No disappearing positive attack was reproduced; no board-behavior
  fix is justified.** Only diagnostic context and an optional original
  menu-input selector were added. See §15 for exact evidence/next step.
- Latest manual run `run_live_20260911_142603`: user reports 1P garbage
  ...
  attack before transport is involved. Capture a specific linked
  two-or-more chain while all four are alive and nothing is being
  countered; do not change original thresholds. At divisor 140, a
  logged conversion input of 280 or more suffices in that clean state.
...
- `PuyoPuyo2/re_notes/COMM_PROTOCOL_SPEC.md` — the full technical record.
  Read this fully; it has 15 numbered sections/updates, each documenting
