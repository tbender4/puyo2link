...
**Latest: plugin 0.5.0, spec Update 7 (section 12).**
...
- Reset-aware `.bin.wire` journals and a mutual FE-ready barrier now
  separate packet generations. Peer reset stops old traffic and lets
  the other cabinet's native watchdog reset it before reconnection.
- **385 callback/ROM assertions pass** using the repository's existing
...
- Shared reset generations are now callback-tested, but recovery can
  wait for the native 1200-tick watchdog and normal game error handling.
  Measure this in the asymmetric-reset recipe; do not force game RAM.
...
Both cabinets must use 0.5.0. Preserve `.bin.wire` journals too: those are
...
- `PuyoPuyo2/re_notes/COMM_PROTOCOL_SPEC.md` — the full technical record.
  Read this fully; it has 12 numbered sections/updates, each documenting
