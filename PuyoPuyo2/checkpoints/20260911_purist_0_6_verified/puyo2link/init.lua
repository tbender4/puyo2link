...
local function do_prestart()
	...
	protection_tap = space:install_write_tap(0xffa020, 0xffa027, 'puyo2link_protection', function(off, data, mask)
		if (space:read_u16(off) & mask) ~= (data & mask) then
	...
	checksum_tap = space:install_write_tap(0xffa0b2, 0xffa0b3, 'puyo2link_checksum', function(off, data, mask)
		log(string.format('[protection-checksum] data=%04x mask=%04x pc=%06x',
			data, mask, cpu.state['PC'].value))
		return nil
	end)
	attack_tap = space:install_write_tap(0xffa7e0, 0xffa7ff, 'puyo2link_attacks', function(off, data, mask)
		if (space:read_u16(off) & mask) ~= (data & mask) then
	...
	divisor_tap = space:install_read_tap(0xffa026, 0xffa027, 'puyo2link_divisor', function(off, data, mask)
		local pc = cpu.state['PC'].value
		if pc >= 0x7848 and pc <= 0x7850 then
			log(string.format('[garbage-calculation] protection=%02x score=%04x divisor=%04x task=%06x pc=%06x players=%d rule=%02x split_remainder=%02x',
