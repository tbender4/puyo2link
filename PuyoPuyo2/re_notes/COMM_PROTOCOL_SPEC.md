...
## 13. Update 8 (2026-09-11): native banked RX, garbage investigation, and real gameplay

**Plugin 0.6.0. This supersedes the execution/gameplay blockers in Updates
6–7 and the assertion that RX banking is ambiguous or requires RAM injection.**
The original ROM, protection checks, game flags, tasks and watchdog are
unmodified. MAME/unidasm execution now works. Backups made **before changes**:

```
PuyoPuyo2\checkpoints\20260911_garbage_purist\
```

This contains the complete 0.5.0 plugin, handoff, notes/tests and all four
original `run_live_20260911_*` directories, including `.wire` journals.
Original logs remain in place too. Do not delete/reuse any of these runs.

### 13.1 What the missing-garbage evidence actually establishes

**Do not claim that RX injection or copy protection caused the user's
reported failure. Neither is established by the evidence, and a control
run with the unchanged 0.5.0 plugin also produced real garbage.**

Read-only decoding of every historical journal, with application sizes
derived from `$183fc-$18426`, found no sequence errors, incomplete packets
or discrepancies between raw `.bin` contents and concatenated wire data:

| Historical run | A→B packets / attack packets | B→A packets / attack packets | Positive attack packets |
|---|---:|---:|---:|
| `run_live_20260911_131804` | 1506 / 32 | 1679 / 50 | 0 / 0 |
| `run_live_20260911_133823` | 735 / 0 | 738 / 0 | 0 / 0 |
| `run_live_20260911_134004` | 777 / 0 | 780 / 0 | 0 / 0 |
| `run_live_20260911_134407` | 1660 / 13 | 3044 / 25 | 0 / 0 |

The gameplay runs' four-byte attack payloads contain only `$8000` and
`$ffff`: zero-quantity completion and no-update markers respectively.
**The sending games never put a positive garbage quantity on those
recorded streams.** This localizes the observed absence upstream of
remote attack delivery; it is not evidence of a dropped or misrouted
positive attack. Historical logs did not record puyo-pop scores or the
protection flag, so they cannot distinguish no/insufficient scoring,
countering, match state, or a historical protection failure.

There was also a **proven instrumentation bug**: actual MAME 0.289 reports
PC `$1845e` for the sequence-increment write at `$18456`. The old filter
ended at `$1845c`, so every historical `accepted=0` and `payloadN=0` is
misleading, not evidence of a dead parser. The filter now includes the
observed PC. Wrapped bit-2 and bit-3 payload bytes are logged correctly.

### 13.2 Protection, original ROM identification and the comparison patch

Inspected the extracted original and patched images, both ZIPs,
`READEM.TXT`, `protectionCode.png`, and
https://mikejmoffitt.com/articles/0047-puyopuy2.html.

`mame.exe puyopuy2 -verifyroms` reports **one ROM set, good**.
`roms\puyopuy2.zip` and `PuyoPuyo2\puyopuy2.zip` contain the same original
ROM bytes matching the driver's expected SHA-1 values:

| ROM | SHA-1 |
|---|---|
| `epr-17241.ic32` (even bytes) | `77fb0482fa35e615c0bed65f4d7f4dd89b241f23` |
| `epr-17240.ic31` (odd bytes) | `c2bdad4b6184c11f81f2a5db409cb4ea186205a7` |
| `epr-17239.ic4` (samples) | `6095b8277b47a6fd7a9721f15a70ae5bf6be9b1a` |

**Important difference from the blog narrative:** this workspace's
patched images differ from the original merged program at only
`$7850-$7853`. The original `move.w #$ffff,D1` garbage-divisor consequence
is replaced by two NOPs. The workspace patch does **not** alter the first
CPLD comparison routine. It is only a comparison clue, not used in any
validation run and not part of the solution.

The real CPLD test is at `$d16` (duplicate `$18824`, reference `$1b13a`).
Its indirect failure store `$ff4816 + $5810` reaches `$ffa026`. The
garbage calculation tests that flag at `$7848`, substitutes divisor
`$ffff` if nonzero, then divides at `$7854`.
The separate register-integrity and ROM-code checks use `$ffa0b2/b3`
(`$1b090`, `$1b0b4`). In the new gameplay runs the CPLD flag stays zero
and the code check **sets then clears** `$ffa0b3` normally.

The plugin only intercepts `$880100-$88013f`; the driver's protection
decode `$800000` with mirror `$13fdfe` does not include this window
(A19 differs). The video-control register `$800201` is separate too.
No protection handler/mirror is replaced. Full-word mailbox reads now
preserve the incoming unwired upper byte; upper-byte-only accesses pass
through unchanged. `cfg\puyopuy2.cfg` contained no DIP overrides. No DIP,
cheat, protection lookup, ROM or gameplay setting was changed.

### 13.3 Four-byte attacks: source counts, not opponent IDs

These are **bit 3** packets, not the fourteen-byte bit-4 field graphics.

- `$da1e-$da72` packs two words from `$ffa7e0/$ffa7e4` into `$ffa532`.
  `$ffff` means no update for that local source player. Bit 15 reflects
  the source's completion state; the low 15 bits are quantity.
- `$50ba-$50c8` marks completion even if a piece produced no garbage.
  Thus a stream of `$8000` packets alone is normal for non-scoring play.
- `$18618-$18652` receives the two words into accumulators: source 1
  adds to `$ffa7f2` **and** `$ffa7fa`, source 2 to `$ffa7f6` **and**
  `$ffa7fe`. These are both receiving local fields' per-source counts.
  `$ffff` skips that source; `$8000` carries zero quantity.
- Local opponent attack accumulation occurs separately at `$755a`.
  The link does not select a single remote opponent by a packet ID.

Native scoring can legitimately produce zero garbage. The new exercise
recorded e.g. score `$004b` and normal divisor `$0078` (75 / 120) with
protection zero. Additionally, the `$7508-$7534` path divides by the
number of other active players and retains the division remainder at
player-task +`$23`; countering runs at `$7482-$74dc`.
These explain why a pop or a nonzero score does not automatically imply
a positive attack. They are **not proof of the exact cause in the user's
historical play**, which lacked this instrumentation.

### 13.4 Native RX implementation (no game-memory writes)

The actual opcodes remove the earlier supposed ambiguity:

1. CONTROL `$FF`, read RxCredit at `$18784`.
2. ROM clamps against its own 255-byte software ring (`$18798-$187ae`).
3. Read CONTROL position at `$187b8`, select
   `$20 | (position >> 3)` at `$187d0`.
4. Read banked slots at `$187d4`; **the original CPU instruction** copies
   them into `$ffa900`, rotating through all 32 RX banks.
5. Return to CONTROL, write semaphore, publish new position at `$18804`,
   and commit consumed count via `sub.b D5,($6,A2)` at `$1880e`.

0.6.0 implements separate 256-byte RX storage, a hardware producer
position and occupancy capped at 255. Frame polling fills only free
hardware storage. Bank/credit reads are repeatable and never pop bytes.
The CPU publishes its consumed position; the subsequent credit write
acknowledges that delta. ACKs are derived from position rather than the
bus subtraction result, so data arriving in a frame callback between a
read-modify-write's read and write cannot disappear. Unacknowledged
storage cannot be overwritten during a burst.

The plugin **contains no `space:write_*` calls at all**. It no longer
reads software RX occupancy to decide delivery, repairs a software ring,
injects bytes, or disables the native RX loop. Software ring handling is
entirely original CPU code. Generation invalidation still reports no RX
availability and backpressures TX; only a native reset reinitializes the
game. Boot signature/CRC/FE replies remain distinct from runtime banks.

`received=` now counts bytes acknowledged by the CPU's native RX loop;
it replaces the obsolete `injected=` diagnostic. `rxcredit=` shows
hardware occupancy. Pump-read counts include the RX ACK instruction's
credit read, so they are not one-for-one interrupt counts.

### 13.5 Actual paired MAME and gameplay validation

Every pair used a **fresh folder**, two separate attached async
PowerShell foreground calls, original `puyopuy2` ROMs, and no game-memory
writes beyond the old injector in the explicit 0.5.0 control experiment.
No unrelated process was stopped. Completed runs reported exit 0 and
approximately 100% speed.

- `run_diag_baseline_20260911`: 45-second pair, original RX injector
  plus read-only flag/sequence taps. Established actual `$1845e` PC;
  no CPLD failure at boot. This was attract-only, not gameplay proof.
- `run_purist_exercise_20260911_01`: 90-second native-RX pair with
  simple input pulses and screenshots. Reached linked fields but
  generated only zero-quantity markers. Not a garbage success claim.
- `run_purist_exercise_20260911_02`: 120-second native-RX pair,
  distinct local starts and simple directional inputs. Real pop
  calculations appeared, but quantities stayed zero. Screenshot and
  trace include 75 / 120 above.
- **`run_purist_exercise_20260911_03`: 120-second native-RX gameplay
  pair with an input-only placement controller.** A→B: 909 packets,
  **16 positive attack packets**; B→A: 890 packets, **11 positive attack
  packets**. Zero stream sequence errors. Receiver logs record all
  16/11 positive attacks respectively, plus native accumulator writes.
  Example B at 56.375 seconds accepts `0002,ffff`, then native PCs
  `$1864c/$18650` add 2 to `$ffa7f2/$ffa7fa`; subsequent native game
  code consumes/counters the amounts. Cabinet clocks have different
  origins; do not directly subtract A/B log times.
  RAM snapshots show mode `$01`, match `$04`, participant mask `$0f`.
  At exercise frame 4200, both local fields on both cabinets contain
  landed garbage (high nibble `$e` cells): **A = 1/8, B = 3/4**.
  Screenshots and dumps persist alongside the logs. This is actual
  four-player gameplay and garbage, not merely an established flag or
  attract-mode heartbeat.
- **`run_original05_exercise_20260911_04`: 120-second control with the
  untouched backed-up 0.5.0 plugin**, selected using `-pluginspath`.
  The first log line confirms version 0.5.0. Same input-only placement
  controller, original ROM. **9/16 positive attack packets** and landed
  garbage also occur. Its accepted counters remain the known broken
  zero. This explicitly prevents attributing recovered garbage to the
  banked-RX conversion: the original failure was not reproduced here.

The input controller reads field/task state and operates actual
coin/start/joystick fields. It never seeds a board, forces a game mode,
changes a score or skips protection. Its purpose is to obtain real pops
and attacks reproducibly; it is **not enabled by the plugin** or by
normal manual launch. Successful runs do not prove every manual setup,
edge case, full match progression, or undocumented physical timing.

### 13.6 Regression coverage and useful diagnostics

The existing GENie/Lua harness now models the **whole native RX pump**
through `$18812`, and gives plugin code memory-write methods that
unconditionally fail the test. This prevents reintroducing RAM injection
or flag writes. Tests cover all byte values, multiple ring/bank wraps,
RX capacity independent of software RAM, partial consumption/full rings,
repeatable reads, mid-copy arrivals, RX and TX credit RMW interleavings,
reset generations, file errors, bus lanes, observed parser PC and wrapped
attack diagnostics. Exact ROM premises now include RX opcodes,
protection consequence and native attack routing.

```
.\src\3rdparty\bx\tools\bin\windows\genie.exe --file=PuyoPuyo2\re_notes\test_transport.lua
PASS: 298 callback assertions (Lua 5.3); no MAME/gameplay execution
```

The count is not directly comparable to 385: the old repeated
“mailbox RX must stay disabled” assertion was intentionally removed.

New read-only records:

- `[rom-guards]`: original comparison/divisor opcodes in loaded memory.
- `[protection-state]`, `[protection-checksum]`: native flag changes.
- `[garbage-calculation]`: flag, score, unmodified divisor, task and
  active-player/rule/remainder context before the original division.
- `[packet-accepted] ... attack=hhhh,hhhh`: actual received source words.
- `[attack-state]`: native outgoing/incoming accumulator writes and PCs.

`py PuyoPuyo2\re_notes\analyze_link.py <run_directory> [...]` independently
audits completed journals and summarizes packet/attack counts. A wire
count alone does not prove application acceptance or a landed attack.

For an optional automated input-only run, keep `PUYO2_AUTO_INPUT=0` and
add `-autoboot_delay 0 -autoboot_script PuyoPuyo2\re_notes\exercise_link.lua`.
It writes input records, screen snapshots and read-only RAM snapshots to
the fresh run folder. Normal user gameplay should **omit this option**.

**Remaining diagnostic target:** if the user still sees no garbage,
reproduce manually with 0.6.0 in a fresh folder and correlate a specific
visible pop/chain with score/divisor/flag, outgoing `$ffa7e*` accumulation,
the transmitted bit-3 word, peer acceptance and native receiving
accumulators. Do not apply the workspace protection patch or force flags
to conceal the missing evidence.

### 13.7 Repeated-reset tap lifetime defect found during live validation

`run_purist_reset_20260911_05` exercised real MAME soft resets. After the
first B reset, A observed the peer generation change at 40.586 seconds,
took its **native** watchdog/reset path at 60.455, and reached matching
generation 2 at 60.796. No watchdog or game state was forced.

The exercise script was reloaded by MAME after soft reset and initially
scheduled a second reset as well. That exposed a separate **proven plugin
defect**: at B time 70.253 one CONTROL write at the same CPU PC ran the
callback twice, publishing generations 3 and 4 together. Adjacent menu
and protection diagnostic writes were duplicated too.

Replacing the Lua tap handle during `do_prestart` only makes the old
userdata collectible; it does not guarantee immediate uninstall.
Until GC, the old tap can still intercept accesses alongside the new
one. MAME's `luaengine_mem.cpp` exposes synchronous `mempassthrough:remove()`.
Prestart now explicitly removes all old mailbox/diagnostic taps before
reinstallation. This also prevents duplicate TX capture after machine
reset. The harness rejects installation over any still-active handle and
asserts all eleven old taps were removed. Do not attribute the historical
no-positive-attack streams to this newly observed reset bug without a
matching trace.

The optional exercise now explicitly unsubscribes its own old callbacks
on reload and limits hardware resets across script reloads. For reset
validation only, set `PUYO2_EXERCISE_RESET_FRAME=2100` on B and optionally
`PUYO2_EXERCISE_RESET_COUNT=2`; normal exercise/manual play uses neither.

**Final live regression: `run_purist_reset_20260911_06`, 105 seconds,
both exits 0.** Two B soft resets now publish exactly generations 2 and
3, one CONTROL callback each (no duplicate generation 4). A detects
the resets at 44.474 and 79.519 seconds, reaches **watchdog=1200** at
native PC `$183ac` at 64.035/99.080, and reconnects at 64.385/99.430:
about **19.91 seconds** after each peer reset. Both endpoints finish
ready in generation 3 with packets being accepted again. All three
generation pairs independently decode from sequence zero with no
sequence errors; raw and wire payloads match. This is reset/transport
validation, not an additional four-player match claim.

The verified plugin, notes and all new validation directories are
checkpointed at `checkpoints\20260911_purist_0_6_verified\`. Historical
live logs/streams were SHA-256 compared against the pre-change
checkpoint and remain byte-identical. The final input helper includes
generation tags in snapshot names to avoid overwriting images/dumps
on script reload.

## 14. Update 9 (2026-09-11): manual 1P versus linked-mode report

The user reports that garbage reaches the AI in 1P but remains absent in
linked play. The corresponding completed manual run is
`run_live_20260911_142603`, plugin 0.6.0, original ROMs. No code or ROM
changes were made in response to this run.

The new calculation diagnostics now explain the specific recorded
linked-mode divisions, rather than merely showing empty attack packets:

| Cabinet B emulated time | Active players | Division input D0 | Normal divisor D1 | Integer quotient | Protection flag |
|---|---:|---:|---:|---:|---:|
| 57.160 | 4 | 80 (`$0050`) | 140 (`$008c`) | 0 | 0 |
| 61.599 | 4 | 136 (`$0088`) | 140 (`$008c`) | 0 | 0 |
| 100.982 | 2 | 485 (`$01e5`) | 120 (`$0078`) | 4 | 0 |
| 114.267 | 2 | 198 (`$00c6`) | 120 (`$0078`) | 1 | 0 |

These are division inputs, not necessarily individual pop scores:
the game retains the division remainder in the player task. Do not
add 80 and 136 as two independent scores. Cabinet A has no recorded
garbage-calculation events in this run.

Re-read the original instructions using unidasm: `$77e4` clears D0,
`$7838` loads the player's score/remainder input, `$7848` tests
`$ffa026`, and `$7854` divides. The zero flag seen in all four records
branches past the punitive `$ffff` divisor. At `$743a-$7440` the caller
stores the division remainder back to task +`$0e`; `$7444-$7446`
returns without creating an attack when the quotient is zero.
Thus both observed four-player calculations stop before countering,
distribution or the communication board can affect an attack.

Distribution is rule-dependent, not universally division by three:
for the logged rule `$01`, with four active players, the later
`$7546-$7550` path doubles a positive post-countering quantity and
divides by three. Rule zero takes the separate opponents-count
division path at `$7516-$7524`. Neither path is reached for the two
zero quotients above. Divisor selection also depends on game state/time;
140 versus 120 here must not be described as a universal 4P/1P constant.

The completed journals match raw streams, have no incomplete packets or
sequence errors, and contain 618 A-to-B / 1330 B-to-A packets.
All 6 / 13 attack packets carry zero/no-update markers; the peer logs
show those markers accepted.

**Conclusion scoped to this run:** the captured four-player clears were
below the original game's current garbage-conversion threshold, with
the protection check passing. This explains these observed zero attacks,
not every historical report or an unrecorded larger chain. The next
useful manual reproduction is a larger linked-mode chain with its
visible outcome correlated to these same diagnostics. Do not lower
thresholds, manufacture attacks, or patch protection to hide the result.

## 15. Update 10 (2026-09-11): matching the manual game's selected profile

**A real setup difference was found, traced through the original ROM, and
reproduced using menu inputs. It is not a communication-board defect.**
The earlier successful exercise selected a different menu item from the
manual run. This investigation did not change transport, protection,
scoring or any original game rule. The plugin changes are read-only
diagnostic fields; the optional exercise gained an input-only selector.
Pre-change files are in `checkpoints\20260911_rule_investigation\`.

### 15.1 The setup packets explain 140 versus 120

In `run_live_20260911_142603`, A's final selection command is `$84,$02`.
In `run_purist_exercise_20260911_03`, it is `$84,$00`. These are not
opponent IDs or a failed ready handshake:

- `$241ac-$2437e` implements a shared, three-position selector. Role A
  changes its index from local joystick input or B's input requests;
  `$24270` broadcasts changes as `$44,index`. `$2428a` confirms with
  `$84,index`; B receives the confirmed index at `$24358`.
- `$24188-$24196` maps that index through ROM `$23f66`:
  **indices 0,1,2 map to internal values 2,0,1**, stored in `$ffa05a`.
- Script `$1a476` calls `$1ab5e`, copying that value to `$ffa105`.
  If the operator preset `$ff4804` is zero (AUTO), `$1ab7a` maps it
  through `$1ab8e` to parameter presets **1,4,6,3**. A separate
  `$ffa138` special flag can override this with preset 11.
- This routine jumps into **`$2444`**, not the `$243e` entry point.
  Missing this interior entry would incorrectly suggest that
  `$ff480d` only changes in the service menu. `$245c` copies the base
  garbage divisor from the five-byte preset table into `$ff480d`.
  Preset 4 (`$2496`) has base **140**; preset 6 (`$24a0`) has **120**.

Thus, with AUTO and no special override:

| Confirmed menu index | `$ffa105` | Parameter preset | Base divisor |
|---|---:|---:|---:|
| 0 (earlier exercise) | 2 | 6 | 120 |
| 2 (manual selection) | 1 | 4 | 140 |

This is an original selectable game-parameter difference, not evidence
that the board negotiated the wrong rules. Do not “fix” it by making
both selections use 120. The ROM's `$1aa86` path can also reselect
presets through `$2444` (including preset 10, base 120); a persisted
NVRAM value after exit does not establish the value during an earlier
linked round. No service-setting edit is necessary to explain the
changing divisor.

Both historical runs really reached match `$04` and participant mask
`$0f`, with the same `$16/$26` and `$28/$38` setup sequence. The
17-byte bit-5 packets with indices 0–23 are **packed piece sequences**,
not a rules structure: `$6338-$6474` builds the three 256-byte tables,
`$64ec-$650a` packs pairs of nibbles into 24 chunks, and
`$18522/$6552-$65a8` receives/unpacks them. Negative/high-bit bit-5
indices use another destination and must not all be labelled decks.
Neither the piece-sequence transfer nor native attack reception writes
the parameter preset or garbage divisor.

### 15.2 Score, clock, countering and routing cross-check

`$50d4` advances the chain count; `$8a74/$8b28` computes and adds the
pop's points to the task's **longword** at +`$0e`, separately updating
the displayed total at +`$0a`. `$5128` calls garbage conversion.
`$743a-$7440` replaces the score accumulator with the division remainder.
Consequently a later division input is not a second independent score.

The round clock at `$ffa064` increments after 60 ticks at `$005ee-$00606`
and is cleared at `$4d5e/$8062`. `$77e8` divides that clock by 16,
applies the mode-dependent offset, then subtracts `$ff4802` before
indexing the margin table. With margin 6 and linked mode, the initial
base remains unchanged for the first 96 game-clock seconds. Transport
delay does not independently overwrite this clock or divisor.

For nonzero quotients, `$7482-$74dc` first counters local task +`$38`,
task +`$14`, then the two relevant remote-source counts. Rule 1 with
four active players applies the `$7546-$7550` **multiply by 2, divide
by 3** path to the remaining quantity plus task +`$23` carry. With no
incoming garbage/carry, input 280 at divisor 140 yields quotient 2,
then one garbage per opponent. A positive first quotient of only one
can still leave no outgoing garbage after this distribution.

`$755a` adds to the other local player's task +`$38`, and `$755e` to the
source's outgoing accumulator. `$da1e-$da72` packs those source counts,
and `$18618-$18652` adds each received source to **both** local fields.
`$75ee-$760c` totals each field's two remote sources and local/pending
counts for its garbage display. There is no single remote-player ID
whose accidental duplication would silently suppress all these paths.
The manual run's zero quotients stop before countering or routing.

### 15.3 Focused instrumentation and input-only reproduction

Existing `[game-state]` records now also include mode, rule, base,
margin, internal difficulty and operator preset. Existing
`[garbage-calculation]` records add those fields, game-clock seconds,
participant mask and chain count. These are reads only, with no new
tap, memory write, packet change or timing rule.

`exercise_link.lua` optionally accepts
`PUYO2_EXERCISE_DIFFICULTY=0|1|2`, meaning **original selector index**,
not a raw divisor. It waits for the native selector task, uses ordinary
Left/Button 1 inputs on A and lets the original protocol confirm on B.
The normal exercise remains unchanged when the variable is absent.
Normal manual launches still omit the exercise script entirely.

The callback suite now passes **303 assertions**, including original
selector/preset table bytes and read-only diagnostic context.
`mame.exe puyopuy2 -verifyroms` still reports the original set good.
The live validation uses separate A/B NVRAM and configuration directories
copied from the existing state, so neither cabinet can overwrite the
user's normal saved settings. No NVRAM bytes were manually edited.

**Completed live runs (both processes exit 0, approximately 100% speed):**

- `run_setupdiag_20260911_15`, 75 seconds: default-selector diagnostic
  control; base 120, **8/7 positive A→B/B→A packets**, all counted by
  the peer. This validates the read-only log extension, not the manual
  failure or a fix for it.
- **`run_manualprofile140_20260911_15`, 100 seconds:** selected index 2
  using Left then Button 1 (`exercise_A.log`, frames 756/792). Native
  confirmation is `$84,$02`, matching the manual capture. Both games
  report **mode 1, match 4, mask 0f, rule 1, internal difficulty 1,
  AUTO preset 0, base/effective divisor 140 and margin 6**.
  There are **10/9 positive A→B/B→A packets**, all accepted by the
  respective native peer parser. For example, B at 25.936 seconds
  accepts `0001,ffff`, followed by native `$1864c/$18650` writes of 1
  to `$ffa7f2/$ffa7fa`. A accepts the reverse `0001,ffff` at 27.506.
  At frame 3000, RAM snapshots show landed high-nibble `$e` garbage
  in **A's fields = 1/1 cells, B's fields = 2/1 cells**; screenshots
  and snapshots persist. These are real attacks at the manual
  selection's 140-point threshold, not only a repeated 120-point test.

Both completed runs have zero journal/application tails, zero sequence
errors and byte-identical raw versus journal payloads. The manual-profile
run contains 799 A→B / 792 B→A packets. SHA-256 checks of the user's
normal `nvram\puyopuy2\nvram` and `roms\puyopuy2.zip` match their
pre-investigation values. All four gameplay-test processes exited;
no unrelated process was stopped.

**Outcome:** the known manual selection/profile is reproduced, but a
positive attack disappearing is **not** reproduced. There is no
justified communication-board behavior fix from this evidence. The
user's symptom remains unresolved beyond the zero-quotient events in
section 14; do not describe the logging or input-helper changes as
fixing missing garbage.

For the next manual report, keep the existing selection and original
rules. While all four players are still alive and no garbage is waiting
to be countered, record a specific two-or-more chain, cabinet/player,
and approximate time. At this 140-point profile, an input of at least
280 with no incoming garbage/carry is sufficient for a positive
distributed attack; use the **logged conversion input**, not the
cumulative score display, to check that condition. Preserve the run's
logs/journals and preferably a screenshot or recording showing the
chain and all relevant fields. A positive calculation/accumulator
followed by an absent outgoing packet, or a positive accepted packet
without the corresponding native receiving/landing path, is the
specific missing evidence needed to identify a remaining defect.

## 16. Update 11 (2026-09-11): user confirmation and cleanup

**The missing-garbage report is closed.** After manually playing cabinet
A against random movement/rotation inputs on B, the user confirmed:
"garbage did indeed work i just didn't play it enough."

The corresponding preserved run is `run_randomB_20260911_152913`.
Its A-to-B stream contains 783 packets, including positive source
quantities 1 and 10 (`0001,ffff` and `000a,ffff`). B-to-A contains
877 packets with zero-quantity completion markers. Both journals match
their raw streams with no incomplete packets or sequence errors.
This manual confirmation complements, rather than replaces, the
bidirectional controlled-gameplay evidence in sections 13 and 15.
No scoring, protection or routing patch was needed to resolve the report.

Plugin **0.6.1** retains the 0.6.0 communication behavior and makes
investigation verbosity opt-in with `PUYO2_LINK_DEBUG=1`:

- Normal mode installs just the two mailbox taps and the read-only
  accepted-packet counter. It retains reset/readiness/error messages
  and a compact transport summary every 300 emulated frames.
- Debug mode restores the eight additional game/protection taps,
  per-packet payload traces and the 60-frame game/task summaries.
- Raw `.bin` streams and generation-tagged `.bin.wire` journals remain
  available in both modes. Diagnostics never write game memory.
- All tap handles, including optional ones, remain retained at module
  scope and synchronously removed before a machine reset reinstalls them.

The optional random-B input helper remains available; see HANDOFF.md.
It presses only Left/Right/Button 1 for active pieces, leaving menus,
coin/start and Down manual. It is not part of the communication board.
Legacy coin/start automation behavior is unchanged and should still be
disabled with `PUYO2_AUTO_INPUT=0` for manual play.

The pre-cleanup plugin, full notes/handoff and confirmed run are backed
up at `checkpoints\20260911_cleanup_0_6_1\`. No old run directories,
checkpoints, ROMs or NVRAM have been deleted or edited. HANDOFF.md now
leads with the confirmed working status instead of an obsolete request
to reproduce missing garbage. Earlier sections remain historical records.

Cleanup regression coverage: the existing transport harness passes 361
assertions, now exercising quiet mode by default and detailed traces
explicitly. It covers exact transmitted bytes, accepted counters, absent
debug taps/logs, and reset transitions between both verbosity modes.
The random-input helper's separate harness also passes.
`run_cleanup_20260911_0_6_1` is a short paired-MAME run with quiet A and
debug B, isolated copies of cabinet settings, and manual inputs disabled
from automation. Both exited normally with accepted packets flowing.
This is a logging/transport regression, not another gameplay claim.

## 17. Update 12 (2026-09-11): mid-match invitation inputs and a separate recruitment race

**Plugin 0.6.1 is unchanged. No ROM patch, protection bypass, game-memory
write, reset, forced game flag, or communication-board behavior change
was made for this investigation.** The user's attract-to-linked-gameplay
and garbage confirmation remains valid. This update investigates the
different case of interrupting a running cabinet.

Two distinct outcomes must not be conflated:

1. **The original invitation defaults to No, and neither Start key
   selects Yes.** Action buttons 1 and 2 also do not mean No and Yes:
   both confirm whichever item is highlighted. Correct selection was
   reproduced through actual four-player gameplay, not just the message UI.
2. **Correct acceptance can still encounter a later recruitment race
   in accelerated paired tests.** That is after the invitation handshake
   succeeds. It is not evidence that the invitation rejected Yes, nor
   does the successful normal-speed case prove all scheduling is correct.

The user said “press 1 or 2,” without establishing whether that meant
keyboard Start keys or arcade action buttons. Both interpretations are
covered below; do not retrospectively assert which they used. Their two
screenshots are historical UI examples, not fresh traces of this run.

### 17.1 Exact original-ROM menu contract

The invitation task is installed at `$ffd040` by `$19b54-$19b68`.
Its saved menu continuation is `$19c6e`. Consequently task +`$2a/+2b`
are `$ffd06a/$ffd06b`: selection and confirmation respectively **while
this task is the invitation**. Later recruitment reuses those same
bytes as a timer; later values such as `$ff,$78` are not invitation
choices.

Relevant instructions:

```
$19c32  move.w #$0141,($28,A0)       ; countdown
$19c78  move.w $fff070,D0           ; P1 processed inputs
$19c7e  tst.b $ffa17d
$19c86  move.w $fff076,D0           ; P2 when P2 initiated
$19c8e  andi.b #$03,D1              ; either action button confirms
$19c92  beq $19cac
$19c94  move.b #1,($2b,A0)          ; same path on timer expiry
$19cac  andi.b #$30,D0              ; otherwise examine vertical direction
$19cb2  btst #5,D0
$19cb8  move.b #1,($2a,A0)          ; Down: Yes
$19cc0  clr.b ($2a,A0)              ; Up: No
```

The cleared initial selection is **0 = いいえ (No)**; **1 = はい (Yes)**.
The `$0141` timer gives approximately 5.36 seconds / 321 emulated
frames before confirmation in the observed traces. **Expiry confirms
the selected item, not unconditionally No.** A selected-Yes timeout
was observed entering the same invitation handshake as explicit confirm.

Move Down first, release, then confirm. The action-button test precedes
the direction test, so pressing Down and confirm for the first time in
the same sampled input can confirm the still-highlighted No.

`src\src\mame\sega\segac2.cpp:801+` maps the player ports as:
Button 1 = `$01`, Button 2 = `$02`, Down = `$10`, Up = `$20`.
Start 1/2 are separate SERVICE-port inputs; processed Start is tested
as bit 3 by other game code (e.g. `$630e/$18f2c`), not by this menu's
`$03` confirmation test.

The helper records MAME's **actual runtime input sequences**, and neither
`cfg\default.cfg` nor `cfg\puyopuy2.cfg` contained input remapping:

| Initiating player | Select Yes | Confirm current choice | Start |
|---|---|---|---|
| P1 | Down arrow | Left Ctrl **or** Left Alt | keyboard `1` |
| P2 | `F` | `A` **or** `S` | keyboard `2` |

These are the recorded top-row `KEYCODE_1/KEYCODE_2` defaults, not a
claim that numeric-keypad digits are mapped identically. Joystick users
use the initiating player's Down and either rotation/action button.
On the subsequent recruitment screen, Start buttons are relevant again.

### 17.2 Invitation acceptance is distinguishable from handshake refusal

For B joining an already-running A:

- `$19a50` waits for confirmation, then `$19a5a` tests selection.
  No takes `$19a62`, sets `$ffa17f=1` and follows the local path.
  **That offer flag alone is not proof of accepting an invitation.**
- Yes takes `$19a7e`, establishes the initiating local player mask, and
  requires peer header bit 6 plus a negative/established `$ffa500`
  before jumping to `$19758`. Losing those prerequisites is a separate
  native fallback, not the menu interpreting Yes as No.
- `$19770` queues command `$01`; B then receives A's `$91`.
  B answers `$21` with its local player count. A changes `$91` to
  `$81`, suspends the old match, runs its joined-message script, and
  eventually clears bit 7 to `$01`. B observes that clear at `$1987c`
  and continues to recruitment. These are actual ROM commands, not
  synthetic backend acknowledgments.
- A's native flow enters `$1996a/$199da`, its active piece tasks stop,
  and script `$1a7fc` runs. B displays **乱入するよ**. Both then run
  recruitment script `$1a842`. This is distinguishable from never
  accepting the prompt, and from a reset-generation failure.

Normal tests used two already-connected processes. A reached an actual
local round with mode `$01`, match `$02`, mask `$03`, and active `$7944`
piece tasks; **mode zero is not required for a local two-player round**.
B was not coined by the helper until A emitted this read-only
active-game marker. No cabinet was restarted or service-reset. Starting
a new peer process or resetting a peer during gameplay is a different
generation/watchdog scenario (section 12), not a required joining step.

### 17.3 Live reproduction and preserved evidence

All runs use fresh directories, separate copied `cfg_A/cfg_B` and
`nvram_A/nvram_B`, debug enabled on both, original ROM, and
`PUYO2_AUTO_INPUT=0`. The new standalone
`re_notes\exercise_latejoin.lua` drives only ordinary input fields,
records mappings/inputs and native read/write observations, and takes
screen and read-only RAM snapshots. It does not modify the existing
startup/garbage exercise or production plugin.

**Successful normal-speed runs:**

- `run_latejoin_yes_normal_08_20260911`: 90 seconds, normal speed.
  A active at frame 1181 / 19.709 seconds. B invitation at frame 1480,
  Down at 1492, Button 1 at 1510, native confirmation at 1511.
  Both sides reach mask `$0f`, match `$04`; both local piece controllers
  on each cabinet are active at frame 2171 / 36.230 seconds and advance
  through subsequent pieces. Frame-3000 screenshots show the local
  fields and remote mini-fields, not an empty recruitment screen.
- `run_latejoin_yes_isolated_12_20260911` repeats that normal-speed
  acceptance with **physical keyboard/mouse/joystick providers disabled**
  and the helper holding all unpressed input fields at zero. It again
  reaches active four-player pieces at frame 2171. The exact helper
  source is copied into this run directory.
- `run_latejoin_yes_delayed_11_20260911` also reaches actual
  mask-`$0f` four-player pieces at normal speed, despite joining B's
  second player later (invitation-relative frame 420 instead of 210).
  This rules out “must start the fourth player at frame 210” as a
  claimed original-game requirement.

The successful explicit-Yes handshake has **choice=1, done=1**, commands
`$01/$91/$21/$81/$01`, then recruitment `$73/$83/$93`, original shared
selector and piece-sequence setup, and the `$16/$26`, `$28/$38` round
flow. A real new four-player round follows the interruption; it is not
continuation of the old two-player field as if nothing happened.

**Input controls:**

- `run_latejoin_button1_05_20260911`: Button 1 without Down confirms
  **choice 0** immediately and never queues the invitation `$01/$21`
  handshake. This matches the documented default-No behavior.
- `run_latejoin_start1_isolated_13_20260911`: invitation at frame 1459;
  Start 1 at 1471 does not confirm or move selection. Native confirmation
  occurs at frame 1780, exactly 321 frames after invitation, with choice 0.
- `run_latejoin_start2_isolated_14_20260911`: invitation at frame 1408;
  Start 2 at 1420 likewise leaves No selected and unconfirmed. Native
  timeout confirms choice 0 at frame 1729, again exactly 321 frames later.
- `run_latejoin_button2_isolated_15_20260911`: invitation at frame 1436;
  Button 2 at 1448 immediately confirms choice 0 at frame 1449.
  `latejoin_B_1616_join_progress.png` shows **PLAYER 1 OF 2**, matching
  the reported local outcome. These three isolated controls contain no
  `$01/$21` invitation handshake and do not interrupt A.
- `run_latejoin_yes_timeout_07_20260911`: Down without an action
  button leaves choice 1; the native timer confirms at frame 1748,
  321 frames after invitation at 1427. The `$01/$91/$21/$81/$01`
  handshake and A's interruption occur. Its later recruitment fails
  for the separate reason in section 17.4.

Early test iterations are preserved rather than silently discarded:
`run_latejoin_yes_20260911_01` had an incorrect **test-only** mode-zero
active-game gate, and the original Start1/Start2/control-Button2 runs
did not all reach an invitation. They are not counted as invitation
coverage. Earlier helpers also left physical input unmasked; an anomalous
early confirmation in `start2_retry_10` is not evidence that Start 2
confirms. Final isolated controls remove that ambiguity. No production
code was changed to compensate for any test-harness issue.

### 17.4 A real later race, not an invitation-input diagnosis

Two **accelerated** (`-speed 2`, approximately 1.8–2x measured) runs,
`run_latejoin_yes_02_20260911` and
`run_latejoin_yes_timeout_07_20260911`, accept Yes and interrupt A,
but then fall back to local play during **recruitment**. This must not
be hidden by reporting only that the joining-message screen appeared.

The second run includes an exact read-only observation of the branch:

```
A 33.662  native parser accepts peer command $73,$01
A 33.679  native parser accepts peer command $83,$34
A frame 2018 / 33.679:
  recruit-peer-read pc=018bda data=8334 mask=ff00 localmask=03
```

`$18bd4` reads the peer command latch `$ffa7a6`.
`$18bda` masks its upper command byte with `$30`, and `$18bde` branches
to local fallback when those bits are zero. `$73 & $30` is `$30`
(two remote participants); **`$83 & $30` is zero**. The original parser
has already replaced the former status command with the next confirmed
setup command before this consumer samples the latch. A falls through
`$18bc2/$18d78` to local two-player play; B's confirmation wait then
expires. Neither wire packet was lost or reordered.

In the successful normal-speed isolated run, the same native read at
A frame 1696 instead sees **`$7301`**; B sees **`$3302`**. Both therefore
count the remote participants before receiving `$83`, OR them into
mask `$0f`, exchange setup, and actually play.

The IPC backend polls per emulated frame and independent processes have
host-scheduling/phase differences. This is a concrete timing-sensitive
integration limitation to investigate, **not yet a proven physical
daughterboard register/credit defect or a known baud-rate specification**.
The captured `$73/$83` receives are even on successive emulated frames;
do not oversimplify it to only “two packets delivered in one frame.”
No packet-aware delays, command filtering, RAM-latch repairs, arbitrary
serial-rate guesses, or ROM changes were installed. Normal-speed
success in these samples does not prove that normal-speed scheduling
can never hit this race.

For a new user failure after selecting visible Yes, retain fresh logs
and record whether **乱入するよ / 乱入されたよ** appeared, followed by
which recruitment screen. If the invitation never leaves No, check
the initiating player's directional and action-button inputs first.
If its choice is 1 and the handshake occurs, inspect the recruitment
command read and timing rather than blaming the menu or protection.

### 17.5 Reproducing and preserving the boundary

For ordinary manual use, omit all exercise scripts. Boot both cabinets
into the same fresh link session; leave B at attract, play A, then coin
and Start B. Select visible **はい** with the correct local Down input,
then separately confirm. Add the remaining player on recruitment with
its Start button. Do not reset either cabinet.

For a bounded automated pair, use the usual side/directory/debug/auto
variables, separate copied configuration/NVRAM paths, and:

```
PUYO2_LATEJOIN_CHOICE=yes
PUYO2_LATEJOIN_RECRUIT_DELAY=210
-seconds_to_run 90 -keyboardprovider none -joystickprovider none
-mouseprovider none -autoboot_delay 0
-autoboot_script PuyoPuyo2\re_notes\exercise_latejoin.lua
```

Launch A and B as separate attached foreground MAME processes in async
shell calls; do not use `-nothrottle` for an invitation correctness
claim. Default speed is 1. The optional test cases are `yes`,
`yes_timeout`, `start1`, `start2`, `button1`, `button2`; the default
second-player delay is 420. These are test inputs, not board options or
forced game state. Physical-input overrides are cleared at shutdown.

Pre-edit handoff/spec and production/settings hashes are preserved in
`checkpoints\20260911_latejoin_investigation`. SHA-256 checks show
`init.lua`, `transport.lua`, the original ROM ZIP, normal user NVRAM,
and `cfg\puyopuy2.cfg` unchanged. Existing transport regression still
passes **361 assertions**, and the random-input helper harness passes.
No scoring or garbage fix was made or needed.

Final validation: all launched test processes exited with status 0.
Normal-speed successful pairs measured approximately 100% emulation
speed. The final isolated successful run contains 679 A-to-B / 702
B-to-A packets, and the normal-speed delayed successful run contains
839 / 868. Both, the accelerated race traces, and final isolated
Start1/Start2/Button2 controls have matching raw-versus-wire payloads,
zero sequence errors, zero incomplete packets/journal tails, and only
generation pair `(1,1)`. The successful isolated run has 23 ordinary
attack-completion packets in each direction, all zero-quantity markers;
this simple placement test is not an additional positive-garbage claim.
Original `mame.exe puyopuy2 -verifyroms` reports one set, good.

**User visual corroboration, received 2026-09-11 at 16:16:** "both current
pairs running are in the 4p mode i think youre getting there". Preserve
this as the user's observation of visible four-player mode during the
investigation. The message does not identify the two run directories
or independently distinguish mid-match entry from starting at attract.
The logged A-active-before-B-coin sequence and subsequent native
handshake/four-player piece traces provide that distinction for the
successful late-join runs above. This feedback does not resolve the
separately documented accelerated recruitment race. No additional
overlapping pair was launched in response; the investigator's bounded
test processes had completed and their evidence remains preserved.

### 17.6 Correlating the user's 15:46:50 non-interruption observation

The user separately reported at **15:46:50**: "latest run didn't
interrupt first one's game something's off". This observation is
consistent with the contemporaneous recorded run; it must not be
overruled by the separate general observation of four-player screens.

Using local filesystem creation/snapshot timestamps, the investigator's
pair active at that wall-clock time was
**`run_latejoin_start2_retry_10_20260911`**:

| Local wall time | Recorded event |
|---|---|
| 15:46:04.445 / .725 | B/A logs opened for this pair |
| 15:46:29.355 | B frame-1493 default invitation screenshot |
| 15:46:29.757 | B frame-1517 screenshot after scripted Start 2 |
| 15:46:44.492 / .772 | B/A frame-2400 screenshots |
| 15:46:54.503 / .788 | B/A frame-3000 screenshots |
| About 15:47:19 | This bounded pair finishes, before the next pairs |

This was explicitly **`PUYO2_LATEJOIN_CHOICE=start2`**, a negative
input control, not the directional-Yes acceptance scenario:

- B frame 1493 / 24.915 emulated seconds: invitation ready,
  selection 0. Frame 1505 / 25.116: scripted **2 Players Start**.
  No scripted Down/Yes selection occurs.
- Frame 1533 / 25.584: native `$19c9a` confirmation write observes
  **oldchoice=0, done=0**. This early confirmation is the previously
  documented anomalous, physically-unmasked trial; it does **not**
  establish that Start 2 is a confirm button. The final isolated
  `start2_isolated_14` control instead waits the full 321 frames and
  confirms No on timeout.
- Neither stream contains invitation commands `$01/$21`, and A never
  enters the invitation `$199xx` interruption flow in this run.
  A's local piece counts advance continuously after its original
  frame-1177 active-game marker, with **match 2 / mask 03**.
- At wall time 15:46:44, A's screenshot shows its existing falling-piece
  game, while B is at its separate local level-selection screen.
  B has real local pieces at frame 2531 / 42.238 emulated seconds,
  approximately wall time **15:46:46.7**; its frame-3000 screenshot
  shows a separate local match, also **match 2 / mask 03**.

Thus, for the time-correlated investigator pair, **the user's visual
non-interruption report is accurate and matches the default-No control**.
It is not a failed Yes acknowledgment, nor evidence that the later
successful Yes runs were merely simultaneous-start tests. This mapping
does not identify any unrelated MAME windows the user might also have
had open. The general “both pairs look 4P” observation remains visual
corroboration only, not proof of how any unlabelled window reached it.
No additional pair was launched in response to this feedback.

### 17.7 Overlapping-window feedback and mandatory serial test operation

At **15:48:35**, the user reported: "so sometimes it interrupts.
sometimes it doesn't the overlapping instances makes this harder to
track". The test presentation caused a genuine attribution problem:
positive acceptance scenarios and intentional negative controls were
visible concurrently. Do not dismiss this as user error or treat an
unlabelled screen as evidence for a particular test.

Filesystem timestamps correlate the overlapping investigator runs around
that observation as follows:

| Run | Scenario | Approximate local wall-clock interval |
|---|---|---|
| `yes_isolated_12` | Correct Down/Yes, normal speed; traced four-player gameplay | 15:47:19–15:48:49 |
| `yes_delayed_11` | Correct Down/Yes, later second-player recruitment; traced four-player gameplay | 15:47:20–15:48:35 |
| `start2_isolated_14` | Intentional Start2/default-No control, no interruption expected | 15:48:29–15:49:02 |

Names above abbreviate the corresponding
`run_latejoin_<name>_20260911` folders. This explains why an observer
could see interrupting and non-interrupting runs side by side. It does
not establish exactly which windows they meant, nor negate the separately
traced post-accept race in accelerated runs.

**Effective immediately for subsequent investigation:**

1. Announce the exact scenario and fresh shared directory before launch.
2. Run **one pair only**. Two cabinets within that pair run concurrently;
   different test pairs must not overlap.
3. Wait until **both** attached foreground MAME processes have fully
   exited before launching the next pair. Independent A/B loops over
   scenarios are unsuitable because one side can start a later scenario
   while the other side's earlier process is still active.
4. If older investigator-owned pairs must be stopped, use their exact
   shell IDs/PIDs, preserve files, and label the run interrupted rather
   than claiming a completed bounded test. Never stop unrelated MAME.
5. Correlate feedback with the announced scenario, selection/handshake
   trace and timestamped screenshots, not just the visible player count.

All investigator-owned bounded processes had already returned exit 0
before this request was received. No additional pair was launched and
no completed run was retrospectively labelled interrupted. Future live
tests must follow the single-pair rule even when several independent
experiments could otherwise be executed faster in parallel.

**Parent process observation received at 16:24:** the parent reported
one current MAME pair, PIDs **41476 and 13028**, and had not stopped
anything. A subsequent read-only `Win32_Process` lookup for those exact
PIDs returned neither process. No process was stopped or launched in
response. Their command lines/run directories were therefore not
recovered; record their scenario as **unidentified**, not as another
Yes success or negative control. Process absence alone does not establish
their exit status or gameplay outcome. Any user observation attributed
to that pair needs its actual scenario/run identity before interpretation.

## 18. Update 13 (2026-09-11): user confirms mid-match joining

The user manually controlled both cabinets in
`run_midjoin_manual_20260911_162947` at normal speed, without automation.
After initially reporting another non-interruption, they recognized that
Down selects Yes and subsequently confirmed:
"joining midgame is stable. pressing down was what had me confused."

**The reported mid-match invitation issue is closed.** Selecting the
default No explained the confusion; no communication-board, ROM, input
mapping or gameplay change was necessary. This joins the user's earlier
confirmation that garbage works with the original game rules.

The early rejected attempt in this run took the native No path:
B's `$ffa17f` write at emulated 50.150 seconds reported PC `$019a6a`,
and neither stream contained the invitation-acceptance handshake at
the time it was inspected. Do not mislabel that attempt as a failed
Yes handshake. The user's later confirmation establishes the practical
outcome after learning the menu controls.

Keep section 17.4's accelerated post-accept recruitment race documented
as a separate limitation; the normal-speed manual confirmation neither
